...
def doc_capture():
    ...
    if act=="capture":
        ...
        if image is not None:
            ...
            count_random=0
            count_att_fail=0
            mycursor.execute("SELECT * FROM ph_attendance where att_date=%s && docid=%s",(cdate,uname))
            rw = mycursor.fetchall()
            for rw1 in rw:
                count_random=rw1[14]
                count_att_fail=rw1[15]
            ...
            mycursor.execute("update ph_attendance set count_random=%s,count_att_fail=%s where att_date=%s && docid=%s",(count_random,count_att_fail,cdate,uname))
            mydb.commit()
